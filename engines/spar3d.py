"""SPAR3D Engine — Stability AI Stable Point-Aware 3D (CVPR 2025).

Model: stabilityai/stable-point-aware-3d  (GATED — set HF_TOKEN env var)
Repo:  https://github.com/Stability-AI/stable-point-aware-3d
Paper: SPAR3D: Stable Point-Aware Reconstruction of 3D Objects from Single Images

SPAR3D adds a first-stage point-cloud diffusion model on top of SF3D's mesh
reconstruction pipeline.  The point cloud conditions the second-stage mesh
prediction, specifically improving backside geometry quality and reducing
baked-in lighting artefacts in the albedo — the #1 limitation of SF3D.

Inference time: ~0.7 s (vs. ~0.5 s for SF3D), UV-unwrapped PBR GLB output.
...
class SPAR3DEngine(Engine):
    """Stable Point-Aware 3D single-image engine (Stability AI).

    Accepts 1 image; returns a UV-unwrapped, PBR-textured GLB in ~0.7s inference.
    Improves backside geometry vs. SF3D via a point-cloud conditioning stage.
    Requires HF_TOKEN env var — model is gated on HuggingFace.
    """
    ...
    def validate_prerequisites(self) -> bool:
        for attempt in range(6):
            ...
            raise RuntimeError("CUDA not available — SPAR3D requires GPU")

        total_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
        if total_gb < 8:
            raise RuntimeError(f"GPU has {total_gb:.1f}GB; SPAR3D needs at least 8GB")
        logger.info(f"GPU memory OK: {total_gb:.1f}GB")

        if not os.getenv("HF_TOKEN"):
            raise RuntimeError(
                "HF_TOKEN env var not set — stabilityai/stable-point-aware-3d is a gated model. "
                "Accept the license at https://huggingface.co/stabilityai/stable-point-aware-3d "
                "then set HF_TOKEN."
            )
        ...
        try:
            ...
            raise RuntimeError(f"spar3d package not installed in container: {exc}")

        logger.info("spar3d imports OK")
        return True
    ...
    def preprocess(self, image_paths: Union[str, List[str]]) -> List[Image.Image]:
        ...
        if len(validated) > 1:
            logger.warning(f"SPAR3D uses 1 image; got {len(validated)}, using first")
        path = validated[0]
        ...
        # Background removal — SPAR3D expects RGBA with alpha mask on subject
        try:
            import rembg
    ...
    def infer(self, images: List[Image.Image]) -> Any:
        """Run SPAR3D inference. Returns (mesh, timestamp)."""
        from spar3d.system import SPAR3D

        image = images[0]

        if self._model is None:
            logger.info(f"Loading SPAR3D model: {self.MODEL_ID}")
            t0 = time.time()
            ...
            logger.info(f"SPAR3D loaded in {time.time()-t0:.1f}s")
        ...
        for i, tier in enumerate(_BAKE_TIERS):
            ...
            try:
                t0 = time.time()
                logger.info(
                    f"SPAR3D inference tier={label}: bake_resolution={tier['bake_resolution']} "
                ...
                logger.info(
                    f"SPAR3D done in {elapsed:.1f}s tier={label} — "
        ...
        raise RuntimeError("SPAR3D inference failed at all quality tiers")

    def postprocess(self, infer_output: Any) -> str:
        """Export UV-textured GLB with mesh repair."""
        import trimesh
        import trimesh.repair

        mesh, timestamp = infer_output
        output_dir = Path("/app/output/spar3d")
        output_dir.mkdir(parents=True, exist_ok=True)

        # Mesh repair: fix winding normals and fill simple holes
        try:
            mesh.fix_normals()
        ...
        out_glb = output_dir / f"spar3d_{timestamp}.glb"
